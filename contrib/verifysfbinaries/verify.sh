...
###   This script attempts to download the signature file SHA256SUMS.asc from lua.org
...
RCSUBDIR="test/"
BASEDIR="https://lua.org/bin/"
VERSIONPREFIX="lua-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'lua-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the LUA Core binary release signing key installed?"
   fi
