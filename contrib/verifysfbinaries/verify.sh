...
###   This script attempts to download the signature file SHA256SUMS.asc from luascoin.org
...
WORKINGDIR="/tmp/luascoin"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://luascoin.org/bin/"
VERSIONPREFIX="luascoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'luascoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the LUASCOIN Core binary release signing key installed?"
   fi
