...
const std::string LUASCOINGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString LUASCOINGUI::DEFAULT_WALLET = "~Default";

LUASCOINGUI::LUASCOINGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
    ...
    platformStyle(platformStyle)
{
    ...
    QString windowTitle = tr("LUASCOIN") + " - ";
#ifdef ENABLE_WALLET
    ...
    QPixmap pixmapLogo(":/images/light/luascoin_logo_horizontal");
...
void LUASCOINGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a LUASCOIN address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and luascoin: URIs)"));
    ...
    aboutAction = new QAction(tr("&About LUASCOIN"), this);
    aboutAction->setStatusTip(tr("Show information about LUASCOIN"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for LUASCOIN"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your LUASCOIN addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified LUASCOIN addresses"));
    ...
    openAction->setStatusTip(tr("Open a luascoin: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the LUASCOIN help message to get a list with possible LUASCOIN command-line options"));
...
void LUASCOINGUI::createToolBars(QWidget* statusbar)
{
    if(walletFrame)
    {
...
void LUASCOINGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool LUASCOINGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool LUASCOINGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void LUASCOINGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void LUASCOINGUI::setWalletActionsEnabled(bool enabled)
{
...
void LUASCOINGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
    trayIcon = new QSystemTrayIcon(this);
    QString toolTip = tr("LUASCOIN client") + " " + networkStyle->getTitleAddText();
...
void LUASCOINGUI::createIconMenu(QMenu *pmenu)
{
...
#ifndef Q_OS_MAC
void LUASCOINGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void LUASCOINGUI::optionsClicked()
{
...
void LUASCOINGUI::showDebugWindow()
{
...
void LUASCOINGUI::showInfo()
{
    toolsAction->setChecked(true);
    walletFrame->gotoToolsPageTab(ToolsPage::TAB_INFO);
}

void LUASCOINGUI::showConsole()
{
    toolsAction->setChecked(true);
    walletFrame->gotoToolsPageTab(ToolsPage::TAB_CONSOLE);
}

void LUASCOINGUI::showGraph()
{
    toolsAction->setChecked(true);
    walletFrame->gotoToolsPageTab(ToolsPage::TAB_GRAPH);
}

void LUASCOINGUI::showPeers()
{
    toolsAction->setChecked(true);
    walletFrame->gotoToolsPageTab(ToolsPage::TAB_PEERS);
}

void LUASCOINGUI::showRepair()
{
    toolsAction->setChecked(true);
    walletFrame->gotoToolsPageTab(ToolsPage::TAB_REPAIR);
}

void LUASCOINGUI::showConfEditor()
{
    GUIUtil::openConfigfile();
}

void LUASCOINGUI::showMNConfEditor()
{
    GUIUtil::openMNConfigfile();
}

void LUASCOINGUI::showBackups()
{
    GUIUtil::showBackups();
}

void LUASCOINGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

void LUASCOINGUI::showPrivateSendHelpClicked()
{
    if(!clientModel)
        return;
...
void LUASCOINGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void LUASCOINGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void LUASCOINGUI::gotoSettingsPage() 
{
    settingsAction->setChecked(true);
    if(walletFrame) walletFrame->gotoSettingsPage();
}

void LUASCOINGUI::gotoToolsPage()
{
    toolsAction->setChecked(true);
    if(toolsAction) walletFrame->gotoToolsPage();
}

void LUASCOINGUI::gotoMasternodePage()
{
    QSettings settings;
    if (settings.value("fShowMasternodesTab").toBool()) {
...
void LUASCOINGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void LUASCOINGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void LUASCOINGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void LUASCOINGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void LUASCOINGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to LUASCOIN network", "", count));
    } else {
...
void LUASCOINGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void LUASCOINGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void LUASCOINGUI::updateHeadersSyncProgressLabel()
{
...
void LUASCOINGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
    ...
    if (walletFrame)
    {
        if(secs < 25*60) // 90*60 in luascoin
        {
...
void LUASCOINGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
    if(!clientModel)
        return;
...
void LUASCOINGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("LUASCOIN"); // default title
...
void LUASCOINGUI::changeEvent(QEvent *e)
{
...
void LUASCOINGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void LUASCOINGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void LUASCOINGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(LUASCOINUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void LUASCOINGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void LUASCOINGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool LUASCOINGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool LUASCOINGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void LUASCOINGUI::setHDStatus(int hdEnabled)
{
    QString theme = GUIUtil::getThemeName();
...
void LUASCOINGUI::setEncryptionStatus(int status)
{
    QString theme = GUIUtil::getThemeName();
    switch(status)
    {
...
void LUASCOINGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void LUASCOINGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void LUASCOINGUI::detectShutdown()
{
...
void LUASCOINGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void LUASCOINGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void LUASCOINGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(LUASCOINGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void LUASCOINGUI::subscribeToCoreSignals()
{
...
void LUASCOINGUI::unsubscribeFromCoreSignals()
{
...
void LUASCOINGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void LUASCOINGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<LUASCOINUnits::Unit> units = LUASCOINUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const LUASCOINUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(LUASCOINUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    Q_FOREACH(LUASCOINUnits::Unit u, LUASCOINUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(LUASCOINUnits::name(u)), this);
...
{
    setText(LUASCOINUnits::name(newUnits));
}
