...
const std::string LUAGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString LUAGUI::DEFAULT_WALLET = "~Default";

LUAGUI::LUAGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
    ...
    platformStyle(platformStyle)
{
    ...
    QPixmap pixmapLogo(":/images/light/lua_logo_horizontal");
...
void LUAGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a LUA address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and lua: URIs)"));
    ...
    aboutAction = new QAction(tr("&About LUA"), this);
    aboutAction->setStatusTip(tr("Show information about LUA"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for LUA"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your LUA addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified LUA addresses"));
    ...
    openAction->setStatusTip(tr("Open a lua: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the LUA help message to get a list with possible LUA command-line options"));
...
void LUAGUI::createToolBars(QWidget* statusbar)
{
    if(walletFrame)
    {
...
void LUAGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool LUAGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool LUAGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void LUAGUI::setWalletActionsEnabled(bool enabled)
{
...
void LUAGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
    trayIcon = new QSystemTrayIcon(this);
    QString toolTip = tr("LUA client") + " " + networkStyle->getTitleAddText();
...
void LUAGUI::createIconMenu(QMenu *pmenu)
{
...
#ifndef Q_OS_MAC
void LUAGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void LUAGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void LUAGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void LUAGUI::gotoSettingsPage() 
{
    settingsAction->setChecked(true);
    if(walletFrame) walletFrame->gotoSettingsPage();
}

void LUAGUI::gotoToolsPage()
{
    toolsAction->setChecked(true);
    if(toolsAction) walletFrame->gotoToolsPage();
}

void LUAGUI::gotoMasternodePage()
{
    QSettings settings;
    if (settings.value("fShowMasternodesTab").toBool()) {
...
void LUAGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void LUAGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void LUAGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void LUAGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void LUAGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to LUA network", "", count));
    } else {
...
void LUAGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void LUAGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void LUAGUI::updateHeadersSyncProgressLabel()
{
...
void LUAGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void LUAGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
    if(!clientModel)
        return;
...
void LUAGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("LUA"); // default title
...
void LUAGUI::changeEvent(QEvent *e)
{
...
void LUAGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void LUAGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void LUAGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(LUAUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void LUAGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void LUAGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool LUAGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool LUAGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void LUAGUI::setHDStatus(int hdEnabled)
{
    QString theme = GUIUtil::getThemeName();
...
void LUAGUI::setEncryptionStatus(int status)
{
    QString theme = GUIUtil::getThemeName();
    switch(status)
    {
...
void LUAGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void LUAGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void LUAGUI::detectShutdown()
{
...
void LUAGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void LUAGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void LUAGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(LUAGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void LUAGUI::subscribeToCoreSignals()
{
...
void LUAGUI::unsubscribeFromCoreSignals()
{
...
void LUAGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void LUAGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<LUAUnits::Unit> units = LUAUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const LUAUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(LUAUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    Q_FOREACH(LUAUnits::Unit u, LUAUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(LUAUnits::name(u)), this);
...
{
    setText(LUAUnits::name(newUnits));
}
