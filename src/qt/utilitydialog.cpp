    ...
    ui(new Ui::HelpMessageDialog)
{
    ...
    QString version = tr("LUASCOIN Core") + " " + tr("version") + " " + QString::fromStdString(FormatFullVersion());
    ...
    if (helpMode == about)
    {
        setWindowTitle(tr("About LUASCOIN Core"));
        ...
        QString header = tr("Usage:") + "\n" +
            "  luascoin-qt [" + tr("command-line options") + "]                     " + "\n";
        ...
        std::string strUsage = HelpMessage(HMM_LUASCOIN_QT);
        ...
        if (showDebug) {
            strUsage += HelpMessageOpt("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", LUASCOINGUI::DEFAULT_UIPLATFORM));
        }
...
All the LUASCOIN in your wallet is comprised of different \"inputs\" which you can think of as separate, discrete coins.<br> \
...
These denominations are 0.01 LUASCOIN, 0.1 LUASCOIN, 1 LUASCOIN and 10 LUASCOIN -- sort of like the paper money you use every day.</li> \
    ...
    QWidget(parent, f)
{
    ...
    layout->addWidget(new QLabel(
        tr("LUASCOIN Core is shutting down...") + "<br /><br />" +
        tr("Do not shut down the computer until this window disappears.")));
    setLayout(layout);
}

QWidget *ShutdownWindow::showShutdownWindow(LUASCOINGUI *window)
{
    if (!window)
        return nullptr;
