    ...
    ui(new Ui::HelpMessageDialog)
{
    ...
    QString version = tr("LUA Core") + " " + tr("version") + " " + QString::fromStdString(FormatFullVersion());
    ...
    if (helpMode == about)
    {
        setWindowTitle(tr("About LUA Core"));
        ...
        QString header = tr("Usage:") + "\n" +
            "  lua-qt [" + tr("command-line options") + "]                     " + "\n";
        ...
        std::string strUsage = HelpMessage(HMM_LUA_QT);
        ...
        if (showDebug) {
            strUsage += HelpMessageOpt("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", LUAGUI::DEFAULT_UIPLATFORM));
        }
...
All the LUA in your wallet is comprised of different \"inputs\" which you can think of as separate, discrete coins.<br> \
...
These denominations are 0.01 LUA, 0.1 LUA, 1 LUA and 10 LUA -- sort of like the paper money you use every day.</li> \
    ...
    QWidget(parent, f)
{
    ...
    layout->addWidget(new QLabel(
        tr("LUA Core is shutting down...") + "<br /><br />" +
        tr("Do not shut down the computer until this window disappears.")));
    setLayout(layout);
}

QWidget *ShutdownWindow::showShutdownWindow(LUAGUI *window)
{
    if (!window)
        return nullptr;
