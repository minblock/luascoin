    ...
    platformStyle(platformStyle)
{
    ...
    // see https://github.com/luascoin/luascoin/issues/5716
...
{
    GUIUtil::setClipboard(LUASCOINUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = LUASCOINUnits::LUASCOIN;
    ...
    l2->setText(LUASCOINUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(LUASCOINUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(LUASCOINUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(LUASCOINUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0 && (coinControl->nMinimumTotalFee < nPayFee))
    {
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(QString, std::vector<COutput>)& coins, mapCoins) {
        ...
        BOOST_FOREACH(const COutput& out, coins.second) {
            ...
            {
                sAddress = QString::fromStdString(CLUASCOINAddress(outputAddress).ToString());

                // if listMode or change => show luascoin address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, LUASCOINUnits::format(nDisplayUnit, out.tx->vout[out.i].nValue));
            itemOutput->setToolTip(COLUMN_AMOUNT, LUASCOINUnits::format(nDisplayUnit, out.tx->vout[out.i].nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, LUASCOINUnits::format(nDisplayUnit, nSum));
            itemWalletAddress->setToolTip(COLUMN_AMOUNT, LUASCOINUnits::format(nDisplayUnit, nSum));
