...
LUASCOIN Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- LUASCOINMiner : Generates coins (if wallet is enabled).
...
However, in open source software such as LUASCOIN Core, where everyone uses
...
pay attention to for reviewers of LUASCOIN Core code.

General LUASCOIN Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: LUASCOIN Core uses tinyformat, which is type safe. Leave them out to avoid confusion
